...
#     names = Yob.objects()
#
#     pivot_table_by_name_year = pd.pivot_table(
#         names, values="birth", index="year", columns="name", aggfunc="sum", fill_value=0
#     )
#     # pivot_table_by_name_year['total'] = pivot_table_by_name_year.sum(axis=1)
#     pivot_table_by_name_year_sex = pd.pivot_table(
#         names,
#         values="birth",
#         index=["year", "sex"],
#         columns="name",
...
#
# #     return data
# pivot_table = names.pivot_table(
#     index=["sex", "name"], columns="year", values="birth", aggfunc="sum", fill_value=0
# )
# pivot_table = pivot_table.reset_index()
#
# # 1. Créer des tables de pivot séparées pour chaque sexe
# pivot_male = names[names["sex"] == "M"].pivot_table(
#     index="name", columns="year", values="birth", aggfunc="sum", fill_value=0
# )
# pivot_female = names[names["sex"] == "F"].pivot_table(
#     index="name", columns="year", values="birth", aggfunc="sum", fill_value=0
# )
#
...
#     return pivot_df.apply(
#         lambda row: {
#             "name": row["name"],
#             "years": [
#                 int(year) for year in pivot_df.columns[1:] if row[year] > 0
#             ],  # Les années où il y a des naissances
...
# annual_diversity = names.groupby("year").apply(
#     lambda x: {
...
# annual_diversity_df = pd.DataFrame(
#     annual_diversity.tolist(), index=annual_diversity.index
# )
...
def get_top_names_between_years(start_year, end_year, top_n):
    ...
    result = {
        "data": {"male": male_data, "female": female_data},
        "info": {
            "start_year": start_year,
            "end_year": end_year,
            "top_n": top_n,
            "top_name": {
                "male": {"name": top_male_name, "total": int(top_male_total)},
                "female": {"name": top_female_name, "total": int(top_female_total)},
            },
        },
    }
